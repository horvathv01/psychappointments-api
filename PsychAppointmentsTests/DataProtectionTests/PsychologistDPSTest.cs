...
public class PsychologistDPSTest
{
    ...
    private PsychologistDataProtectionService? _psychologistDPS;
    ...
    { 
        ...
        _psychologistDPS = new PsychologistDataProtectionService();
    ...
    [Test]
    public async Task TestSessionAccessOwn()
    {
        //user is associated with session
        var query = async () => await _sessions.GetById(1);
        var queryResult = await _psychologistDPS.Filter(_psychologist, query);
    ...
    [Test]
    public async Task TestSessionAccessAssociatedPsych1IsPartner()
    {
        //user is associated with session
        var query = async () => await _sessions.GetById(4);
        var queryResult = await _psychologistDPS.Filter(_psychologist, query);
    ...
    [Test]
    public async Task TestSessionAccessAssociatedPsych3IsPartner()
    {
        //user is associated with session
        var query = async () => await _sessions.GetById(3);
        var queryResult = await _psychologistDPS.Filter(_psychologist, query);
    ...
    public async Task TestSessionAccessNotAssociated()
    {
        ...
        bool result =
            ...
            queryResult.PsychologistId == unFiltered.Psychologist.Id &&
            queryResult.Blank == unFiltered.Blank &&
            queryResult.PartnerPsychologistId == null &&
            queryResult.ClientId == null &&
            queryResult.Price == null &&
            queryResult.Frequency == "" &&
            queryResult.SlotId == null &&
    ...
    [Test]
    public async Task TestSlotAccessOwnSlot()
    {
        ...
        bool result =
            ...
            queryResult.Id == unFiltered.Id &&
            queryResult.Id == unFiltered.Id &&
            AssessSessionIds(queryResult.SessionIds);
    ...
    [Test]
    public async Task TestSlotAccessAssociatedPsych1IsPartner()
    {
        //user is associated with slot    
        var query = async () => await _slots.GetById(4);
        var queryResult = await _psychologistDPS.Filter(_psychologist, query);
    ...
    [Test]
    public async Task TestSlotAccessAssociatedPsych3IsPartner()
    {
        //user is associated with slot    
        var query = async () => await _slots.GetById(3);
        var queryResult = await _psychologistDPS.Filter(_psychologist, query);
    ...
    public async Task TestClientAccessOwnClient()
    {
    ...
    public async Task TestClientAccessAssociated()
    {
        ...
        var query = async () => await _users.GetByEmail("client4" + "@psychappointments.com");
        var queryResult = await _psychologistDPS.Filter(_psychologist, query);
    ...
    public async Task TestPsychologistAccessPsychologistIsSelf()
    {
        ...
        bool result =
            ...
            queryResult.DateOfBirth == unFiltered.DateOfBirth.ToString() &&
            queryResult.Address.Equals(unFiltered.Address) &&
            queryResult.Password == unFiltered.Password &&
            queryResult.SessionIds.Count == ((Psychologist)unFiltered).Sessions.Count &&
            //AssessSessionIds(queryResult.SessionIds) &&
            queryResult.PsychologistIds == null &&
            queryResult.ClientIds.Count == ((Psychologist)unFiltered).Clients.Count &&
            queryResult.SlotIds.Count == ((Psychologist)unFiltered).Slots.Count &&
            queryResult.SessionIds.Count > 0 &&
            queryResult.PsychologistIds == null &&
            queryResult.ClientIds.Count > 0 &&
            queryResult.SlotIds.Count > 0 &&
            queryResult.LocationIds == null;
    ...
    [Test]
    public async Task TestPsychologistAccessPsychologistIsPartner()
    {
        //user is not associated with psychologist
        var query = async () => await _users.GetByEmail("psychologist3" + "@psychappointments.com");
        var queryResult = await _psychologistDPS.Filter(_psychologist, query);
    ...
    [Test]
    public async Task TestPsychologistAccessPsychologistIsNotAssociated()
    {
        //user is not associated with psychologist
    ...
    private bool AssessSessionIds(List<long> ids)
    {
        //only the ones that are found among _psychologist's sessions (sessions are added to both main and partner psychologists)
        foreach (var id in ids)
        {
            var session = _psychologist.Sessions.FirstOrDefault(ses => ses.Id == id);
            if (session == null)
            {
                if (!session.Blank)
                {
                    //we can still see blank sessions of other psychologists
                    Console.WriteLine($"Session {id} was not found in {_psychologist.Name}'s list of sessions. ");
                    return false;    
                }
