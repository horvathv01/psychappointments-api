...
private InMemoryLocationRepository? _locations;
    ...
    public async Task TestSessionAccessNotAssociated()
    {
        ...
        bool result =
            ...
            queryResult.Price == 0 &&
            queryResult.Frequency == Enum.GetName(typeof(SessionFrequency), SessionFrequency.None) &&
            queryResult.SlotId == null &&
    ...
    private async Task<bool> AssessSessionIds(List<long> ids)
    {
        ...
        {
            var session = _client.Sessions.FirstOrDefault(ses => ses.Id == id);
            if (session == null)
            {
                var currentSession = await _sessions.GetById(id);
                if (!currentSession.Blank)
                {
                    Console.WriteLine($"Session {id} was not blank, nor client's session and we still saw it. "); 
                    return false;
                }
